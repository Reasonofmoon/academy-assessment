...
YOUR SLICE
  sample(items[level==3], domains={vocabulary,grammar,reading}) — up to 1 each

ROLE
  Competent L3 examinee. Read stem (+ passage if reading). Choose 0-3.
  Do not invent options. If unsolvable, still pick best guess and note why.

OUTPUT
  workspace/lanes/L3.json — one lane_output for level 3
